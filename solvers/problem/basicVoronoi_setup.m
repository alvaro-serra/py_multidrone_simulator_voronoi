% Script of problem setup for basic mav collision avoidance
% 
...
pr.state.maxVx      = 3.0;          % m/s
pr.state.maxVy      = 3.0;          % m/s
pr.state.maxVz      = 1.0;          % m/s
% control input 
pr.input.maxRoll    = deg2rad(20);  % rad, maximum roll, phi
pr.input.maxPitch   = deg2rad(20);  % rad, maximum pitch, theta
...
index.z.slack       =   5:6 ;       % slack, [slack_env, slack_coll]
index.z.pos         =   7:9 ;       % position, [x, y, z]
index.z.vel         =   10:12;      % velocity, [vx, vy, vz]
index.z.euler       =   13:15;      % euler angles, [phi, theta, psi]
...
index.p.weights     =   15:18;      % [w_wp, w_input, w_coll, w_slack]
