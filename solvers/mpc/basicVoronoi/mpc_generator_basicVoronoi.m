% Script for generating FORCES PRO NLP solvers for basic mav collision 
% avoidance
% 
...
% 
%	(x-x1)^2/(a+a1)^2 + (y-y1)^2/(b+b1)^2 + (z-z1)^2/(c+c1)^2 >= 1
% 
% Environment boundaries are considered.
% 
% Slack variables are used for environment boundary constraint and
...
%   z = [phi_c, theta_c, vz_c, psi_rate_c, slack_env, slack_coll, x, y, z, 
%       vx, vy, vz, phi, theta, psi].                       (15)
% where
%   s = [slack_env, slack_coll].                            (2)
...
model.ineq = @(z, p) mpc_nonlinIneq_basicVoronoi(z, p, model.nObs);
% upper/lower bound for inequalities
model.hl = [-1, -1, 0.2,    zeros(1, model.nObs),       zeros(1, model.nObs)]';
model.hu = [ 1,  1,   1,    +inf*ones(1, model.nObs),   +inf*ones(1, model.nObs)]';

%% Objective function
model.objective  = @(z, p) mpc_objective_basicVoronoi(z, p, model.nObs);
model.objectiveN = @(z, p) mpc_objectiveN_basicVoronoi(z, p, model.nObs);
...
codeoptions.maxit       = 600;      % maximum number of iterations
...
codeoptions.parallel    = 0; %1;        % run prediction on multiple cores 
